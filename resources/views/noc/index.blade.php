        ...
        <div class="table-container overflow-x-auto">
            ...
            <div class="flex justify-between items-center mb-4">
                <form method="GET" action="{{ route('noc.protocol.search') }}" class="flex w-full mr-4">
                    <input id="search" class="ml-4 shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" type="search" name="search" placeholder="Pesquisar protocolo / unidade...">
                    ...
                    @if(request()->has('search') && request()->input('search') !== '')
                        <a href="{{ route('noc.index') }}" class="ml-2 bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">Limpar</a>
                    @endif
                </form>
            </div>
                ...
                @foreach($ticketFacilities as $call)
                    <tr class="odd:bg-gray-50">
                            ...
                            <td class="whitespace-nowrap px-4 py-2 text-gray-700">
                                <form action="{{ route('noc.protocol.update', $call->id) }}" method="POST">
                                    ...
                                    <div class="flex justify-center">
                                        <input type="text" name="protocol" placeholder="Digite o protocolo" class="rounded-l-md px-2 py-1 focus:outline-none focus:ring-2 focus:ring-blue-500 border border-gray-300">
                                        <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white rounded-r-md px-4 py-1 focus:outline-none focus:ring-2 focus:ring-blue-500">Atualizar</button>
                                    </div>
                                </form>
                            </td>
                        @endif
                        ...
                        <td class="whitespace-nowrap px-4 py-2 text-gray-700">
                            ...
                            @else
                                <form class="inline-block" action="{{ route('noc.protocol.destroy', $call) }}" method="POST" onsubmit="return confirm('Você está finalizando este chamado. Essa ação não poderá ser desfeita. Continuar?');">
                                    @csrf
                                    <input type="hidden" name="_method" value="DELETE">
                                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                    <button type="submit" class="inline-block rounded bg-indigo-600 px-2 py-1 text-xs font-medium text-white hover:bg-indigo-700">
